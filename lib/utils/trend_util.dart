...
class TrendingUtil {
  static htmlToRepo(String res) {
    try {
      res = res.replaceAll(new RegExp('\n'), '');
    } catch (err) {}
    final repos = [];
    final splitWithH3 = res.split('<article');
    splitWithH3.removeAt(0);
    for (int i = 0; i < splitWithH3.length; i++) {
      final repo = TrendingRepoModel.empty();
      final html = splitWithH3[i];
